...
import { Bell, LayoutDashboard, LogOut, Settings, Shapes } from "lucide-react";
    ...
    <div className="min-h-screen bg-taskly-background text-taskly-foreground">
      <div className="flex min-h-screen">
        <aside className="hidden w-64 shrink-0 border-r border-taskly bg-taskly-surface p-5 lg:block">
          ...
          <button className="mt-10 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-slate-400 transition hover:bg-slate-800 hover:text-slate-100">
          ...
          <header className="flex h-16 items-center justify-between border-b border-taskly px-5 sm:px-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={isPending}
                onClick={() => logout()}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-taskly-brand-soft text-taskly-brand transition hover:bg-[#132b63]"
              >
                <LogOut size={16} />
              </button>
            </div>
